...
def contour_is_square(contour, img_area):
    peri = cv2.arcLength(contour, True)
...
def find_bounding_square(contour, img_area):
    peri = cv2.arcLength(contour, True)
    area = cv2.contourArea(contour)
...
def prune_centers(centers, img_width):
    while True:
        num_centers = len(centers)
        centers = prune_centers_helper(centers, img_width)
        if num_centers == len(centers):
            return centers
    return []

def prune_centers_helper(centers, img_width):
    dist_thresh = 0.25
    pruned_centers = []
    if len(centers) < 4:
        return []
...
def group_centers(centers, img_width):
    if len(centers) <= 1:
        return centers
    dist_thresh = 0.12
    clusters = []
...
def straight_grid_score(points):    
    cube_dim = 3
    grid_points = [[[0,0] for x in range(3)] for y in range(3)] 
...
def find_shapes(img):
    ...
    edges = auto_canny(img_small, 0.2)
    ...
    for contour in contours:
        detected_square = find_bounding_square(contour, img_area)
    ...
    pruned_centers = prune_centers(centers, cols/resize_factor)
    ...
    grouped_centers = group_centers(pruned_centers, cols/resize_factor)
    ...
    for gc in grouped_centers:
        ...
        if len(gc) >= 9 and len(gc) <= 14:
            for c in itertools.combinations(gc, 9):
                ...
                grid_scores.append([straight_grid_score(c), c])
                grid_scores.append([tilted_grid_score(c), c])
            grid_scores.sort()
